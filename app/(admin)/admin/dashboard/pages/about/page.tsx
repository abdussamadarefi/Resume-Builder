...
export default function AboutPageCMS() {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      {/* Core Values (4 Cards) */}
      <Card title="4 Core Value Cards" icon={Heart}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {about.values.map((v: any, idx: number) => (
            <div key={v.id} className="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800/80 space-y-2.5 shadow-sm dark:shadow-none">
              <span className="text-[11px] font-bold text-slate-500">Card #{idx + 1} ({v.icon_name})</span>
              <input
                type="text"
                value={v.title}
                onChange={(e) => {
      ...
      {/* Creator Profile */}
      <Card title="Creator & Maintainer Profile" icon={User}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormField
            label="Creator Name"
