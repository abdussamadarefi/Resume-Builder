...
export default function TemplatesPageCMS() {
  ...
  const [content, setContent] = useDraftState('content/templates-page.json', templatesPageDefault);
              ...
              {content.filter_tabs.map((tab: any, idx: number) => (
                <div key={tab.id} className="p-3 rounded-xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 flex items-center justify-between shadow-sm dark:shadow-none">
                  ...
                  <input
                    ...
                    className="px-2.5 py-1 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-800 rounded text-xs text-slate-900 dark:text-white shadow-sm dark:shadow-none focus:outline-none focus:border-blue-500"
                  />
                </div>
              ))}
            </div>
