...
export default function TemplatesRegistryCMS() {
  ...
  const [templates, setTemplates] = useDraftState('content/templates.json', templatesDefault);
  const [saved, setSaved] = useState(false);

  const handleStage = () => {
    stageFile('content/templates.json', JSON.stringify(templates, null, 2), `Templates Registry (${templates.length} Templates)`);
    setSaved(true);
          ...
          <div
            ...
            className={`p-5 rounded-2xl border transition-all space-y-4 shadow-sm dark:shadow-none ${
              tpl.enabled
                ...
                : 'bg-slate-100/70 dark:bg-slate-950/40 border-slate-200 dark:border-slate-800/40 opacity-70'
            }`}
          >
            {/* Header */}
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div
                  className="w-8 h-8 rounded-xl flex items-center justify-center text-white text-xs font-bold shadow-sm flex-shrink-0"
