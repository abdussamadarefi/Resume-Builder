    ...
    >
      <div className="flex items-center p-4 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors" onClick={onToggle}>
        ...
        <div className="flex-1">
          <h4 className="font-bold text-slate-900 dark:text-white">{item.title || "Untitled Entry"}</h4>
          <p className="text-sm text-slate-500 dark:text-slate-400">{item.subtitle || "No subtitle"}</p>
        </div>
        <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); onRemove(); }} className="text-slate-400 hover:text-red-500 hover:bg-red-500/10 h-8 w-8 ml-2" title="Delete Entry">
          <Trash2 size={16} />
        </Button>
      </div>
